...
static void pimoroni_display_two_st7789_cmd(uint8_t cmd, const uint8_t* data, size_t len) {
...
void pimoroni_display_two_st7789_caset(uint16_t xs, uint16_t xe) {
    uint8_t data[] = {
        xs >> 8,
        xs & 0xff,
        xe >> 8,
    ...
    pimoroni_display_two_st7789_cmd(0x2a, data, sizeof(data));
}

void pimoroni_display_two_st7789_raset(uint16_t ys, uint16_t ye) {
    uint8_t data[] = {
        ys >> 8,
        ys & 0xff,
        ye >> 8,
    ...
    pimoroni_display_two_st7789_cmd(0x2b, data, sizeof(data));
}
...
void pimoroni_display_two_st7789_init(const struct st7789_config* config, uint16_t width, uint16_t height) {
    ...
    pimoroni_display_two_st7789_cmd(0x01, NULL, 0);
    sleep_ms(150);

    // SLPOUT (11h): Sleep Out
    pimoroni_display_two_st7789_cmd(0x11, NULL, 0);
    sleep_ms(50);

    // COLMOD (3Ah): Interface Pixel Format
    // - RGB interface color format     = 65K of RGB interface
    // - Control interface color format = 16bit/pixel
    pimoroni_display_two_st7789_cmd(0x3a, (uint8_t[]){ 0x55 }, 1);
    sleep_ms(10);
    ...
    pimoroni_display_two_st7789_cmd(0x36, (uint8_t[]){ 0x00 }, 1);
   
    pimoroni_display_two_st7789_caset(0, width);
    pimoroni_display_two_st7789_raset(0, height);

    // INVON (21h): Display Inversion On
    pimoroni_display_two_st7789_cmd(0x21, NULL, 0);
    sleep_ms(10);

    // NORON (13h): Normal Display Mode On
    pimoroni_display_two_st7789_cmd(0x13, NULL, 0);
    sleep_ms(10);

    // DISPON (29h): Display On
    pimoroni_display_two_st7789_cmd(0x29, NULL, 0);
    sleep_ms(10);
...
void st7789_fill(uint16_t pixel)
{
    ...
    pimoroni_display_two_st7789_set_cursor(0, 0);
...
{
    pimoroni_display_two_st7789_caset(x, st7789_width);
    pimoroni_display_two_st7789_raset(y, st7789_height);
}
...
void st7789_vertical_scroll(uint16_t row)
{
    ...
    pimoroni_display_two_st7789_cmd(0x37, data, sizeof(data));
}